...
const Profile = () => {
  ...
  const [showCreatePost, setShowCreatePost] = useState(false);
  const [postText, setPostText] = useState('');
  const [editPostId, setEditPostId] = useState(null); // State to manage editing
  ...
  const handleSubmit = async () => {
    if (editPostId) {
      await handleDeletePost(editPostId);
    }
  ...
  return (
    <div className="flex h-screen">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  {post.postedBy === user._id && (
                    <div className="flex space-x-2">
                      <button className="text-blue-500" onClick={() => handleEditPost(post)}>
                        <svg
                          className="w-6 h-6"
                          fill="none"
                          viewBox="0 0 24 24"
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M11 5h2m-2 4h2m-1 2h.01M5 11h2M5 15h2M19 7h-4.586a1 1 0 00-.707.293l-7 7A1 1 0 006 16.414V17h.586a1 1 0 00.707-.293l7-7A1 1 0 0018 9.414V9h1V7z"
                          />
                        </svg>
                      </button>
      ...
      {showCreatePost && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-4 rounded shadow-lg">
            <h2 className="text-lg font-bold mb-4">{editPostId ? 'Edit Post' : 'Create Post'}</h2>
            <textarea
              className="w-full p-2 border border-gray-300 rounded mb-4"
              rows="4"
              value={postText}
